			...
			>
				...
				>
					...
					{:else}
						...
						<div
							class="relative h-48 bg-gradient-to-br from-green-50 to-slate-100 flex items-center justify-center text-slate-400"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
...
{#if loading && initialLoadAttempted && posts.length > 0}
	<div class="flex justify-center py-8" aria-live="polite">
		<div class="relative">
			<div class="h-10 w-10">
				<div
					class="absolute animate-spin rounded-full h-10 w-10 border-4 border-slate-200"
				></div>
				<div
					class="absolute animate-spin rounded-full h-10 w-10 border-4 border-green-600 border-t-transparent"
				></div>
			</div>
		</div>
	</div>
...
{#if !loading && !hasMore && initialLoadAttempted}
	<div class="text-center py-8 text-slate-500">
		...
		{:else if !errorMessage}
			<div class="py-16 bg-slate-50 rounded-xl border border-slate-200">
				<svg
					xmlns="http://www.w3.org/2000/svg"
